...
#
# Порядок шагов: проверка Docker, создание .env из .env.example с генерацией
# недостающих секретов (§14.2), выпуск самоподписанного сертификата для nginx
# (без него ingress не стартует), ОЧИСТКА — остановка прежнего стека и удаление
# старого образа шлюза, СБОРКА БЕЗ КЭША (--no-cache --pull), подъём сервисов и
# ожидание готовности шлюза.
#
# Кэш сборки не используется намеренно: слой builder ставит зависимости из
# requirements.txt, и кэшированный слой дал бы образ со старым набором пакетов.
#
...
Использование: ./docker-start.sh [команда] [опции]
  ...
  (без аргументов)         очистить старый образ проекта и собрать заново без
                           кэша (--no-cache --pull), затем поднять стек
  --no-build              поднять существующий образ: очистка и сборка пропускаются
  --profile NAME          включить профиль compose (например discovery)
  down                    остановить контейнеры (тома, образы, .env и сертификаты сохраняются)
  -h, --help              эта справка

Очищаются только образы, собранные этим compose-проектом: базовые образы
(postgres, redis, nginx, prometheus) не удаляются — их может переиспользовать
другой проект на той же машине.
...
if [ "$ACTION" = "down" ]; then
    ...
    step 2 2 "Останавливаю стек…"
    compose down
    note "тома (pgdata, redisdata, metricsdata), образы, $ENV_FILE и $TLS_DIR сохранены"
    exit 0
fi
...
step 2 6 "Проверяю конфигурацию окружения…"
...
step 3 6 "Проверяю сертификаты ingress…"
...
# --------------------------------------------------------------------------- #
# 4. Очистка: прежний стек и старый образ проекта
# --------------------------------------------------------------------------- #

# Образ пересобирается с тем же тегом, поэтому сначала освобождаем его:
# контейнеры стека останавливаются (тома и данные БД не трогаем — без -v), затем
# удаляются образы, собранные ЭТИМ compose-проектом. Базовые образы не
# удаляем: на общей машине их может переиспользовать другой проект (например
# redis:7-alpine из чужого контейнера), а docker compose down --rmi и prune
# работают шире, чем нужно.
if [ "$BUILD" = "1" ]; then
    step 4 6 "Останавливаю прежний стек и удаляю старый образ проекта…"
    compose down --remove-orphans --timeout "${STOP_TIMEOUT_SECONDS:-30}"
    compose_project=$(compose config 2>/dev/null | sed -n 's/^name: //p' | head -1)
    [ -n "$compose_project" ] || fail "не удалось определить имя compose-проекта"
    old_images=$(docker image ls -q --filter "label=com.docker.compose.project=${compose_project}" | sort -u || true)
    if [ -n "$old_images" ]; then
        removed=0
        for image in $old_images; do
            docker image rm "$image" >/dev/null 2>&1 && removed=$((removed + 1))
        done
        note "удалено образов проекта ${compose_project}: ${removed}"
    else
        note "образов этого проекта не найдено — удалять нечего"
    fi
else
    step 4 6 "Пропускаю очистку и удаление образов (--no-build)…"
    note "поднимаю существующий образ проекта"
fi

# --------------------------------------------------------------------------- #
# 5. Сборка без кэша и подъём
# --------------------------------------------------------------------------- #

if [ "$BUILD" = "1" ]; then
    step 5 6 "Собираю образ шлюза без использования кэша…"
    # --no-cache: слой builder ставит зависимости из requirements.txt, кэш дал бы
    # образ со старым набором пакетов. --pull: базовый python:3.12-slim берётся
    # актуальный, а не тот, что закеширован локально.
    compose build --no-cache --pull
else
    step 5 6 "Поднимаю сервисы без пересборки образа…"
fi
